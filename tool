查詢不同版本瀏覽器可否適用css html5
https://caniuse.com/#home


清除瀏覽器預設css html格式
https://meyerweb.com/eric/tools/css/reset/

可以線上丟連結秀code

https://codepen.io/pen/tour/welcome/start


button css套版
https://purecss.io/

css色碼
https://www.toodoo.com/db/color.html


各國瀏覽器使用比率統計
http://gs.statcounter.com/vendor-market-share/mobile#monthly-201806-201906-bar


跨瀏覽器測試
http://browsershots.org/

免費圖床
https://imgur.com/

jquery下載
https://jquery.com/

jquery辭典
https://oscarotero.com/jquery/

動態css第三方插件 Animate.css
https://daneden.github.io/animate.css/

fontawesome用語法載入icon非圖片檔,為文字檔案,
改變尺寸改font-size非改高和寬,搭配jQuery載入
https://fontawesome.com/icons/alarm-clock?style=light
在css檔案裡要載入script檔
<script src="https://kit.fontawesome.com/d038f356df.js"></script>

jQuery plugin
1.要載入js檔(有線上也有下載放到自己資料夾)
2.通常<script>主文件放最後因為希望載完html後在載入jQ效果
3.有些<script>語法可結合入JQUERY寫法裡
https://lokeshdhakar.com/projects/lightbox2/#options
http://idangero.us/swiper/

多個插件(plugin)
1.jquery
2.插件plugin
3.自己寫的js (all.js)
css一樣
<script>寫在body下面

免費icon網站 有提供ai/png/svg
https://www.iconfinder.com/

PNG壓縮網站
https://tinypng.com/

Sass   【】.scss
https://prepros.io/

假大頭照網站
https://randomuser.me/photos

圖表js(要注意商業授權)
chart.js有RWD
https://www.chartjs.org/
https://www.highcharts.com/

免費圖庫
https://unsplash.com/s/photos/jewelry

線上Framkwork mobile
http://jqmdesigner.appspot.com/

[十分鐘學習] Hover.css - 強化系游標移入動畫效果
https://ithelp.ithome.com.tw/articles/10196535
Hover.css v2
https://ianlunn.github.io/Hover/

卡片輪播
https://owlcarousel2.github.io/OwlCarousel2/
https://www.wfublog.com/2016/04/owl-carousel-jquery-slider-cdn.html
http://www.htmleaf.com/jQuery/Slideshow-Scroller/201801064917.html
https://codepen.io/khadkamhn/pen/RxVKXP
